...
// Prevent unhandled Firebase Admin credential rejections from crashing the process
process.on('unhandledRejection', (reason: any) => {
  if (reason?.message?.includes('Could not load the default credentials') || reason?.message?.includes('ADC')) {
    console.warn('[Server] Firebase Admin credentials not available - running without admin features');
    return;
  }
...
${urls.join('\n')}
    ...
    // Run initial generation once with timeout
    ...
    withTimeout(rebuildSitemap(), 8000, '[Sitemap Generator] Initial rebuild').catch(
      (e: any) => console.warn('[Sitemap Generator] Initial rebuild skipped:', e.message)
    );
    ...
    setInterval(() => {
      ...
      withTimeout(rebuildSitemap(), 8000, '[Sitemap Generator] Periodic rebuild').catch(
        (e: any) => console.warn('[Sitemap Generator] Periodic rebuild skipped:', e.message)
      );
    }, 180000);
  };
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  app.post('/api/fcm/register', async (req, res) => {
    ...
    try {
      ...
      // Save under fcm_tokens collection
  ...
  app.post('/api/notifications/send', async (req, res) => {
    ...
    try {
      ...
      // Use correct database via helper
      const dbAdmin = getAdminDb();
...
Our team will follow up via phone or email to deliver custom themes, timeline schedules, and a final contract.`;
  ...
  app.listen(PORT, '0.0.0.0', () => {
    ...
    // Run sitemap generator after server starts (non-blocking)
    if (firebaseApp) {
      setTimeout(() => {
        try {
          setupSitemapGenerator();
        } catch (e: any) {
          console.warn('[Sitemap Generator] Skipped:', e.message);
        }
      }, 1000);
    }
  });
