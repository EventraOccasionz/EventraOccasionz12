...
export const bookingService = {
  ...
  async getRSVPs(): Promise<RSVP[]> {
    ...
    try {
      ...
      console.warn('Failed parsing local RSVPs:', e);
    }
    ...
    if (authService.isConfigured()) {
      ...
      try {
        ...
        console.warn('Failed fetching RSVPs from Firestore:', error);
      }
    }
    ...
    for (const [key, rsvpsList] of grouped.entries()) {
      if (rsvpsList.length === 1) {
        ...
        if (bestRSVP.family_id && bestRSVP.id !== bestRSVP.family_id && authService.isConfigured()) {
          const oldId = bestRSVP.id;
          bestRSVP.id = bestRSVP.family_id;
          try {
            await setDoc(doc(db, 'rsvp', bestRSVP.family_id), bestRSVP);
    ...
    if (duplicateDocIdsToDelete.length > 0 && authService.isConfigured()) {
      duplicateDocIdsToDelete.forEach(docId => {
        deleteDoc(doc(db, 'rsvp', docId)).catch(err => {
  ...
  async submitRSVP(rsvp: Partial<RSVP>): Promise<void> {
    const docId = rsvp.id || rsvp.family_id || (authService.isConfigured() ? doc(collection(db, 'rsvp')).id : 'rsvp_' + Math.random().toString(36).substring(2, 9));
    const cleanData = {
      ...rsvp,
      id: docId,
    ...
    // Cache in sessionStorage for robust reload state persistence
    try {
      const localRSVPs = JSON.parse(sessionStorage.getItem('local_rsvps') || '[]');
      const filtered = localRSVPs.filter((r: any) => r.family_id !== rsvp.family_id && r.id !== docId);
      filtered.push(cleanData);
      sessionStorage.setItem('local_rsvps', JSON.stringify(filtered));
    } catch (e) {
      console.warn('Failed to cache RSVP in sessionStorage:', e);
    }
  ...
  async getTransports(): Promise<TransportRequest[]> {
    ...
    try {
      ...
      if (error?.message?.includes("Missing or insufficient permissions")) {
        console.warn(`[Permission Restricted] Skip transports Firestore query, falling back to local list.`);
        return [];
      }
  ...
  async getRooms(): Promise<RoomBooking[]> {
    ...
    try {
      ...
      if (error?.message?.includes("Missing or insufficient permissions")) {
        console.warn(`[Permission Restricted] Skip rooms Firestore query, falling back to local list.`);
        return [];
      }
      handleFirestoreError(error, OperationType.GET, path);
      return [];
  ...
  async getInquiries(): Promise<Inquiry[]> {
    ...
    try {
      ...
      if (error?.message?.includes("Missing or insufficient permissions")) {
        console.warn(`[Permission Restricted] Skip inquiries Firestore query, falling back to local list.`);
        return [];
      }
      handleFirestoreError(error, OperationType.GET, path);
      return [];
