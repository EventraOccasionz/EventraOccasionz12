...
import { loadEventExperienceConfig, resolvePlaceholders } from '../lib/eventExperience';
...
export default function InvitePage() {
  ...
  const [formData, setFormData] = useState({
    ...
    accommodation_stay: false
  });

  const defaultWeddingEvents = ['Haldi', 'Mehndi & Sangeet', 'Wedding Ceremony', 'Grand Reception'];
  const eventList = eventConfig?.hasSavedConfig
    ? (eventConfig?.scheduleItems || [])
        .filter((item: any) => item.visible !== false && item.rsvpEnabled !== false)
  ...
  useEffect(() => {
    const checkAccess = async () => {
      let data;
      if (slug && slug.startsWith('preview-')) {
        const previewEventId = slug.substring('preview-'.length);
        data = {
          id: 'preview_family',
          name: 'Preview Guest',
          access_code: 'PREVIEW',
      ...
      try {
        ...
        if (existingRSVP) {
          ...
          setFormData({
            ...
            customAnswers: existingRSVP.customAnswers || {},
            accommodation_stay: existingRSVP.accommodation_stay || false
          });
      ...
      try {
        ...
        if (matched) {
          ...
          // Settings are stored with this exact event in event_details/{eventId}.
          setEventConfig(await loadEventExperienceConfig(matched));
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (family.id === 'preview_family') {
      alert('This is a preview mode RSVP submission. No data was saved.');
      setRsvpCompleted(true);
      return;
    }
    setSubmitting(true);
    ...
    const transportCreatedAt = originalTransportCreatedAt || now;
      ...
      const scheduleResponses = eventConfig?.hasSavedConfig
        ? (eventConfig?.scheduleItems || []).reduce((acc: any, item: any) => {
            if (item.rsvpEnabled !== false) {
              acc[item.id] = {
                attending: formData.events.includes(item.title || item.name),
                titleSnapshot: item.title || item.name
              };
            }
            return acc;
          }, {})
        : eventList.reduce((acc: any, evName: string) => {
            acc[evName.toLowerCase().replace(/[^a-z0-9]/g, '_')] = {
              attending: formData.events.includes(evName),
              titleSnapshot: evName
            };
            return acc;
          }, {});
      ...
      await dataService.submitRSVP({
        ...
        accommodation_stay: formData.accommodation_stay || false
      });
...
============================================================
  ...
  // Content remains event-scoped; templates receive the same event with safe configured title overrides.
  const guestEvent = eventConfig?.invitationContent?.title
    ? { ...eventDetails, name: resolvePlaceholders(eventConfig.invitationContent.title, eventDetails, family) }
    : eventDetails;

  const greeting = family && eventConfig?.invitationContent?.personalizedGreeting
    ? resolvePlaceholders(eventConfig.invitationContent.personalizedGreeting, eventDetails, family)
    : family?.name || '';
  ...
  if (normalizedEventType !== 'wedding') {
    return (
      <GuestExperienceRouter 
        ...
        event={guestEvent} 
        slug={slug || ''} 
        experienceConfig={eventConfig || undefined}
      />
    );
  }
          ...
          >
            ...
            >
              ...
              <h1 className="font-serif text-3xl sm:text-5xl text-cream tracking-tight mb-4 text-balance">
                Welcoming The <br />
                <span className="text-gold italic font-bold block mt-1 first-letter:uppercase">{greeting}</span>
              </h1>
          ...
          viewDashboard ? (
            ...
            <section className="min-h-screen relative flex items-center justify-center overflow-hidden py-16 px-6">
              ...
              <div className="relative z-10 text-center max-w-4xl mx-auto flex flex-col items-center">
                ...
                >
                  {eventConfig?.invitation?.invitationMessage || "We request the honor of your presence as we unite our lives forever."}
                </motion.p>
            ...
            {eventConfig?.sectionVisibility?.hero !== false && (
            ...
            {eventConfig?.sectionVisibility?.invitationMessage !== false && (
            ...
            {eventConfig?.sectionVisibility?.timeline !== false && (
              <section className="py-24 px-6 relative bg-gradient-to-b from-[#130002] to-black">
                <div className="max-w-4xl mx-auto text-center">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-left max-w-6xl mx-auto">
                    ...
                    {eventConfig?.hasSavedConfig ? eventConfig.scheduleItems.filter((f: any) => f.visible).map((func: any, idx: number) => (
                      <motion.div 
                        key={func.id}
            ...
            {eventConfig?.sectionVisibility?.venue !== false && (
            ...
            {eventConfig?.sectionVisibility?.accommodation !== false && (
            ...
            {eventConfig?.sectionVisibility?.contact !== false && (
            ...
            {eventConfig?.rsvpSettings?.enabled !== false && eventConfig?.sectionVisibility?.rsvp !== false && (
              <section id="rsvp-section" className="py-20 px-6 max-w-4xl mx-auto relative z-10">
                <div className="text-center mb-12">
                  ...
                  {eventConfig?.rsvpSettings?.deadline && (
                    <p className="text-[10px] text-gold uppercase tracking-widest mt-2 font-bold">
                      RSVP Deadline: {new Date(eventConfig.rsvpSettings.deadline).toLocaleDateString([], { month: 'long', day: 'numeric', year: 'numeric' })}
                    </p>
                  )}
                </div>
                ...
                {rsvpCompleted ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-10 bg-gradient-to-b from-[#220205] to-black border border-gold/25 p-6 md:p-10 rounded-2xl">
                    ...
                    {/* Attendance Section */}
                    {eventConfig?.rsvpSettings?.allowAttending !== false && (
                      <div className="space-y-5">
                        ...
                        {formData.attending === 'yes' && eventConfig?.rsvpSettings?.askAttendeeNames !== false && (
                          <motion.div
                    ...
                    {formData.attending === 'yes' && (
                      ...
                      >
                        ...
                        {eventConfig?.rsvpSettings?.askDietaryPreference !== false && (
                        ...
                        {eventConfig?.rsvpSettings?.askAccommodation === true && (
                          <div className="flex items-center gap-3 bg-black/40 p-4 border border-gold/15 rounded-xl mt-3">
                             <input 
                               type="checkbox" 
                               id="accommodation-req-luxury"
                               className="accent-gold w-4.5 h-4.5 cursor-pointer"
                               checked={formData.accommodation_stay}
                               onChange={e => setFormData({...formData, accommodation_stay: e.target.checked})}
                             />
                             <label htmlFor="accommodation-req-luxury" className="text-xs text-cream/80 cursor-pointer select-none">
                               We request accommodation / hotel room stay arrangements.
                             </label>
                          </div>
                        )}
                      </motion.div>
                    ...
                    {/* Travel & Cab Service */}
                    {formData.attending === 'yes' && (eventConfig?.hasSavedConfig ? !!eventConfig?.rsvpSettings?.askTransport : true) && (
                       <motion.div 
                         initial={{ opacity: 0 }}
                         animate={{ opacity: 1 }}
                         className="space-y-5"
                    ...
                    {/* Notes Section */}
                    {formData.attending === 'yes' && (eventConfig?.hasSavedConfig ? !!eventConfig?.rsvpSettings?.allowMessage : true) && (
                    ...
                    {formData.attending === 'yes' && eventConfig?.customQuestions && eventConfig.customQuestions.length > 0 && (
                      <div className="space-y-5 pt-4 border-t border-gold/15">
                        <h4 className="text-[10px] uppercase tracking-[0.25em] text-gold pb-2 border-b border-gold/15">Custom Questions</h4>
                        {eventConfig.customQuestions.map((q: any) => (
                          <div key={q.id} className="flex flex-col gap-2">
                            <label className="text-[9px] uppercase tracking-wider text-[#D4AF37]/75 font-semibold">
                                ...
                                {['Yes', 'No'].map(opt => (
                                  <button
                                    ...
                                    onClick={() => setFormData(prev => ({
                                      ...prev,
                                      customAnswers: { ...(prev.customAnswers || {}), [q.id]: opt }
                                    }))}
                                    className={`flex-1 py-3 border rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all
                                      ${formData.customAnswers?.[q.id] === opt ? 'bg-gold text-black border-gold' : 'bg-white/5 border-white/10 text-white/40 hover:border-gold/30'}`}
                                  >
                                    {opt}
                                  </button>
                                ))}
                              ...
                              <div className="flex flex-wrap gap-2.5">
                                {q.options?.split(',').map((opt: string) => {
                                  ...
                                  const isSelected = q.type === 'single'
                                    ? formData.customAnswers?.[q.id] === cleanOpt
                                    : Array.isArray(formData.customAnswers?.[q.id]) && formData.customAnswers[q.id].includes(cleanOpt);
                                  return (
                                    <button
                                      key={cleanOpt}
                                      type="button"
                                      onClick={() => {
                                        if (q.type === 'single') {
                                          setFormData(prev => ({
                                            ...prev,
                                            customAnswers: { ...(prev.customAnswers || {}), [q.id]: cleanOpt }
                                          }));
                                        } else {
                                          const currentList = Array.isArray(formData.customAnswers?.[q.id]) ? [...formData.customAnswers[q.id]] : [];
                                          const newList = currentList.includes(cleanOpt)
                                            ? currentList.filter(item => item !== cleanOpt)
                                            : [...currentList, cleanOpt];
                                          setFormData(prev => ({
                                            ...prev,
                                            customAnswers: { ...(prev.customAnswers || {}), [q.id]: newList }
                                          }));
                                        }
                                      }}
                                      className={`py-2 px-3 border rounded-xl text-[10px] uppercase tracking-widest transition-all font-bold
                                        ${isSelected ? 'bg-gold text-black border-gold' : 'bg-white/5 border-white/10 text-white/40'}`}
                                    >
                                      {cleanOpt}
                                    </button>
                                  );
            ...
            {eventConfig?.sectionVisibility?.closing !== false && (
              <section className="py-16 px-6 relative bg-gradient-to-t from-[#120002] to-black text-center border-t border-gold/10">
                <div className="max-w-md mx-auto">
                  <span className="text-gold tracking-[0.35em] text-[9px] uppercase block mb-3 font-mono">✦ Shubh Vivah ✦</span>
                  <p className="text-sm font-serif text-cream italic leading-relaxed">
                    {eventConfig?.invitationContent?.closingMessage || "Thank you for being a part of our beautiful journey. We look forward to your blessings and presence."}
                  </p>
                </div>
              </section>
            )}
