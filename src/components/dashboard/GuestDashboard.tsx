...
import { 
  ...
  Car, Hotel, Map, Volume2, VolumeX, Phone, 
  Clock, Users, Smartphone, FileText, Send, Share2, 
  CheckCircle2, Sparkles, Info, ArrowLeft, ArrowRight, Lock,
  Bell, X
} from 'lucide-react';
...
export default function GuestDashboard({
  ...
  const [readNotifIds, setReadNotifIds] = useState<string[]>(() => {
    const cached = localStorage.getItem(`read_notifs_${family.id}`);
  ...
  const markAllAsRead = () => {
    ...
    localStorage.setItem(`read_notifs_${family.id}`, JSON.stringify(allIds));
  };

  const sortedAnnouncements = [...announcements].sort((a, b) => {
    const priorityScore = { urgent: 3, important: 2, normal: 1 };
    const aScore = priorityScore[a.priority as 'urgent' | 'important' | 'normal'] || 1;
    const bScore = priorityScore[b.priority as 'urgent' | 'important' | 'normal'] || 1;
    if (aScore !== bScore) {
        ...
        <div className="flex justify-between items-center border-b border-gold/10 pb-6 mb-8">
          ...
          <div className="flex gap-2 items-center">
            ...
            <button 
              ...
              className="p-2.5 bg-white/5 border border-gold/20 text-gold rounded-xl hover:bg-gold/10 transition-all relative shrink-0"
              title="Notification Center"
            >
              <Bell size={18} className={unreadCount > 0 ? "animate-bounce" : ""} />
              {unreadCount > 0 && (
                <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-red-500 text-white text-[9px] font-mono font-bold flex items-center justify-center border border-[#070002] shadow-lg">
                  {unreadCount}
                </span>
              )}
            </button>
            ...
            >
              ...
              {announcements.length > 0 && (
                <div 
                  ...
                  className="bg-gold/5 border border-gold/30 rounded-2xl p-5 relative overflow-hidden flex items-start gap-4 shadow-lg cursor-pointer hover:bg-gold/10 transition-all"
                >
                  ...
                  <div className="space-y-1 w-full">
                    <div className="flex justify-between items-center">
                      ...
                      {unreadCount > 0 && (
                        <span className="bg-red-500 text-white text-[8px] font-mono font-bold px-1.5 py-0.5 rounded-full animate-pulse">
                          {unreadCount} New
                        </span>
                      )}
                    </div>
        ...
        <AnimatePresence>
          {isNotifCenterOpen && (
            ...
            >
              ...
              <div className="absolute inset-0" onClick={() => setIsNotifCenterOpen(false)} />
              ...
              <motion.div
                ...
                className="relative w-full max-w-md h-full bg-[#070002] border-l border-gold/25 shadow-2xl flex flex-col z-10"
              >
                {/* Decorative border gradient */}
                <div className="absolute left-0 top-0 bottom-0 w-[1px] bg-gradient-to-b from-transparent via-gold/40 to-transparent" />

                {/* Header */}
                <div className="p-6 border-b border-gold/10 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="w-9 h-9 rounded-full bg-gold/5 border border-gold/20 flex items-center justify-center text-gold">
                      <Bell size={16} />
                    </span>
                    <div>
                      <h3 className="font-serif text-lg text-cream tracking-tight">The Royal Announcements</h3>
                      <p className="text-[9px] text-gold uppercase tracking-widest font-mono">Wedding Broadcast Center</p>
                    </div>
                  </div>
                  <button
                    onClick={() => setIsNotifCenterOpen(false)}
                    className="p-2 text-[#FDFBF7]/60 hover:text-white hover:bg-white/5 rounded-xl transition-all"
                  >
                    <X size={18} />
                  </button>
                </div>
                ...
                <div className="flex-grow overflow-y-auto p-6 space-y-4">
                  {loadingAnnouncements ? (
                    <div className="h-full flex flex-col items-center justify-center gap-3">
                      ...
                      <p className="text-xs text-text-secondary font-mono">Retrieving updates...</p>
                    </div>
                  ) : announcements.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-center py-16 px-4">
                      <Bell className="text-white/10 mb-3" size={36} strokeWidth={1} />
                      <h5 className="font-serif text-sm text-gold">Serene Stillness</h5>
                      <p className="text-[11px] text-text-secondary font-sans leading-relaxed mt-1 max-w-xs">
                        All is peaceful. announcements dispatched by your gracious hosts will instantly arrive in this sacred gallery.
                      </p>
                    </div>
                  ) : (
                      ...
                      return (
                        <div
                          ...
                          className={`p-4 rounded-xl border relative transition-all duration-300 ${
                            isUrgent
                              ? 'bg-red-500/5 border-red-500/30 hover:border-red-500/50 shadow-[0_4px_12px_rgba(239,68,68,0.05)]'
                        ...
                        >
                          ...
                          <div className="flex justify-between items-center gap-2 mb-2">
                            <span className={`px-2 py-0.5 rounded uppercase text-[8px] font-bold font-mono tracking-wider border ${
                              isUrgent 
                          ...
                          <h5 className="text-sm font-serif font-bold text-cream mb-1">
                            {item.title || 'Wedding Announcement'}
                          </h5>
                          <p className="text-xs text-cream/80 font-sans leading-relaxed">
                            {item.message}
                          </p>

                          <div className="mt-3 pt-2 border-t border-white/[0.03] flex justify-between items-center text-[9px] font-mono text-white/30">
                ...
                {/* Footer */}
                <div className="p-4 border-t border-gold/10 text-center bg-black/40">
                  <p className="text-[9px] text-[#FDFBF7]/40 uppercase tracking-widest font-mono">
                    ✦ Shubh Vivah Broadcast Center ✦
                  </p>
                </div>
              </motion.div>
            </motion.div>
