...
interface EventsTabProps {
  ...
  onSelectEvent: (id: string, tab?: string) => void;
...
export default function EventsTab({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const isBabyShower = type === 'Baby Shower';

    let isValid = !!(name && date && venue);
    
    if (isWedding || isEngagement || isAnniversary) {
      isValid = !!(isValid && bride && groom && familyName);
    } else if (isBirthday || isBabyShower) {
      isValid = !!(isValid && bride && familyName);
    } else if (isCorporate) {
      isValid = !!(isValid && familyName);
    }
    ...
    // Prepare data - safely clear irrelevant fields
    const finalBride = (isWedding || isEngagement || isAnniversary || isBirthday || isBabyShower) ? bride : '';
    const finalGroom = (isWedding || isEngagement || isAnniversary || isBabyShower) ? groom : '';
  ...
  if (activeEvent) {
    const initialTab = (activeEvent as any)._openExperienceTab ? 'experience' : 'overview';
    return (
        ...
        <div className="fixed inset-0 z-[5000] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="w-full max-w-2xl bg-[#141414] border border-gold/20 rounded-2xl p-6 sm:p-8 relative shadow-2xl my-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
              ...
              {/* Participant Info - Dynamically Rendered based on Event Type */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {/* Primary Name Field (Bride / Birthday Person / Partner 1 / Mother / Company) */}
                {(type === 'Wedding' || type === 'Engagement' || type === 'Anniversary' || type === 'Birthday' || type === 'Baby Shower' || type === 'Corporate') && (
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] uppercase tracking-widest text-gold/80 font-mono font-bold">
                      {type === 'Wedding' ? 'Bride Name *' : 
                       type === 'Birthday' ? 'Birthday Person Name *' : 
                       type === 'Corporate' ? 'Company Name *' : 
                       type === 'Anniversary' ? 'Partner / Person 1 Name *' : 
                       type === 'Baby Shower' ? 'Mother / Parent Name *' : 
                       type === 'Engagement' ? 'Partner 1 / Bride Name *' : 
                       'Host Name'}
                    </label>
                    <input
                      type="text" required
                      ...
                      value={type === 'Corporate' ? familyName : bride}
                      onChange={e => type === 'Corporate' ? setFamilyName(e.target.value) : setBride(e.target.value)}
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-xs text-cream outline-none focus:border-gold transition-colors"
                    />
                  </div>
                )}

                {/* Secondary Name Field (Groom / Partner 2 / Father / Co-host) */}
                {(type === 'Wedding' || type === 'Engagement' || type === 'Anniversary' || type === 'Baby Shower') && (
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] uppercase tracking-widest text-gold/80 font-mono font-bold">
                      {type === 'Wedding' ? 'Groom Name *' : 
                       type === 'Engagement' ? 'Partner 2 / Groom Name *' : 
                       type === 'Anniversary' ? 'Partner / Person 2 Name *' : 
                       'Father / Co-host Name'}
                    </label>
                    <input
                      type="text" 
                      required={type !== 'Baby Shower'}
                      placeholder={type === 'Wedding' ? 'Groom Name' : 'Name'}
                      value={groom}
                ...
                {/* Family Name Field (Hidden for Corporate since re-using for Company Name) */}
                {type !== 'Corporate' && type !== 'Other' && (
                  <div className="flex flex-col gap-1.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 border-t border-white/5 pt-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] uppercase tracking-widest text-gold/80 font-mono font-bold">
                    {type === 'Corporate' ? 'Organizer Name' : 'Client Name'}
                  </label>
                  <input
                    type="text"
                    placeholder={type === 'Corporate' ? 'Contact Person' : 'Client Contact Name'}
                    value={clientName}
            ...
            return (
              ...
              >
                ...
                <div className="space-y-2 mt-auto">
                  ...
                  <button 
                    onClick={() => {
                      onSelectEvent(ev.id, 'experience');
                    }}
                    className="w-full bg-gold/20 text-gold py-2.5 px-3 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-gold/30 transition-all border border-gold/40 flex items-center justify-center gap-2"
                  >
