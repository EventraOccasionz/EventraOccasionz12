...
export default function BirthdayExperience({ family, currentEvent }: BirthdayExperienceProps) {
      ...
      <div className="relative z-10 max-w-2xl mx-auto">
        ...
        >
          ...
          {currentEvent.date && (
            <div className="mt-8 bg-black/20 border border-amber-400/10 rounded-2xl p-4 max-w-md mx-auto backdrop-blur-sm">
              <span className="text-[9px] uppercase tracking-[0.25em] text-amber-300/60 font-mono block mb-2">COUNTDOWN TO CELEBRATION</span>
              <BirthdayCountdown targetDate={currentEvent.date} />
            </div>
          )}
        </motion.div>
        ...
        >
          ...
          <div className="space-y-6">
            {[
              { time: '07:30 PM', name: 'Grand Entrance & Welcome Drink', desc: 'Sip on handcrafted custom cocktails while welcoming guests to the red-carpet reception.' },
              { time: '08:15 PM', name: 'Cake Cutting Ceremony & Roasts', desc: 'A grand celebration with high-definition laser lighting shows and heartfelt messages.' },
              { time: '09:00 PM', name: 'Lively Dance & Entertainment', desc: 'A custom energetic musical session led by high-profile guest DJs & entertainment artists.' },
              { time: '10:00 PM', name: 'Luxury Buffet Dinner', desc: 'Indulge in a curated international luxury dinner menu.' }
            ].map((prog, idx) => (
              <div key={idx} className="flex gap-4 items-start pb-5 border-b border-white/5 last:border-0 last:pb-0">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#FDFBF7]/70 mt-1 font-mono leading-relaxed">{prog.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        >
          ...
          <h3 className="text-lg font-bold text-amber-100">RSVP Gate</h3>
          <p className="text-xs text-[#FDFBF7]/70 font-mono mt-1.5 max-w-sm mx-auto leading-relaxed">
            The digital RSVP system for this special event will open shortly. We are working to coordinate hotel details, travel desks, and catering preferences.
          </p>
          <div className="mt-4 flex justify-center gap-4">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-400/10 border border-amber-400/30 text-[10px] text-amber-300 font-mono">
