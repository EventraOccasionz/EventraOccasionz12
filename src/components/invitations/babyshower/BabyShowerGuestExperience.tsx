...
export default function BabyShowerGuestExperience({ family, event, eventId, experienceConfig }: BabyShowerGuestExperienceProps) {
  ...
  return (
    <div id="babyshower-guest-portal" className="bg-[#FAF8F5] min-h-screen text-[#2C3830] font-sans flex flex-col justify-between relative overflow-x-hidden selection:bg-[#5D6E63]/20 selection:text-[#1F2922]">
      ...
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 pt-8 pb-20 relative z-10 flex-grow">
        ...
        {event?.description && experienceConfig?.sectionVisibility?.about !== false && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          ...
          {experienceConfig?.sectionVisibility?.accommodation !== false && (
            roomBooking ? (
              <div className="bg-white border border-[#E8EDE9] p-6 rounded-3xl shadow-sm relative overflow-hidden flex flex-col justify-between min-h-[190px]">
                <div>
                  <div className="w-10 h-10 rounded-xl bg-[#FAF8F5] border border-[#E8EDE9] text-[#5D6E63] flex items-center justify-center mb-4 shadow-inner">
          ...
          {experienceConfig?.sectionVisibility?.transport !== false && (
            transport ? (
              <div className="bg-white border border-[#E8EDE9] p-6 rounded-3xl shadow-sm relative overflow-hidden flex flex-col justify-between min-h-[190px]">
                <div>
                  <div className="w-10 h-10 rounded-xl bg-[#FAF8F5] border border-[#E8EDE9] text-[#5D6E63] flex items-center justify-center mb-4 shadow-inner">
              ...
              {attending && (
                ...
                >
                          ...
                          return (
                            ...
                            >
                              ...
                              <div>
                                ...
                                <p className="text-[10px] font-mono text-[#2C3830]/60 mt-1">{slot.time || slot.startTime}</p>
                              </div>
                            </div>
                          );
                        })}
                  ...
                  {experienceConfig?.rsvpSettings?.askAccommodation === true && (
                    <div className="flex items-center gap-3 bg-[#FAF8F5] border border-[#E8EDE9] p-4 rounded-xl mt-3">
                      <input 
                        type="checkbox" 
                        id="accommodation-stay-check"
                        disabled={isRsvpLocked}
                        className="accent-[#5D6E63] w-4 h-4 cursor-pointer"
                        checked={accommodationStay}
                        onChange={e => setAccommodationStay(e.target.checked)}
                      />
                      <label htmlFor="accommodation-stay-check" className="text-xs text-[#2C3830] cursor-pointer select-none">
                  ...
                  {experienceConfig?.customQuestions && experienceConfig.customQuestions.length > 0 && (
                    <div className="space-y-4 pt-4 border-t border-[#E8EDE9]">
                      <h4 className="text-[#2C3830]/60 text-[10px] uppercase tracking-wider font-mono font-bold">Custom Questions</h4>
                      {experienceConfig.customQuestions.map((q: any) => (
                        <div key={q.id} className="flex flex-col gap-1.5">
                          <label className="text-[#1F2922] text-xs font-medium">
                          ...
                          {q.type === 'long' && (
                            <textarea
                              ...
                              disabled={isRsvpLocked}
                              rows={3}
                              className="w-full bg-[#FAF8F5] border border-[#E8EDE9] focus:border-[#5D6E63] rounded-xl p-3 text-[#1F2922] outline-none transition-all leading-relaxed"
                              value={customAnswers?.[q.id] || ''}
                              onChange={e => setCustomAnswers(prev => ({
                                ...prev,
                                [q.id]: e.target.value
                                ...
                                <button
                                  ...
                                  className={`py-2 rounded-lg border text-center font-bold tracking-wider uppercase font-mono text-[10px] transition-all cursor-pointer ${
                                    customAnswers?.[q.id] === opt 
                                      ? 'bg-[#5D6E63] text-white border-[#5D6E63]' 
                                      : 'bg-[#FAF8F5] border-[#E8EDE9] text-[#2C3830]/60 hover:border-[#5D6E63]/30'
                                  }`}
                                >
                                  {opt}
