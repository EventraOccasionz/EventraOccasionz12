...
export default function BirthdayGuestExperience({ family, event, eventId, experienceConfig }: BirthdayGuestExperienceProps) {
  ...
  const handleSaveRsvp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const rsvpPayload = {
        ...
        events: attending ? timeline.filter((item) => item.rsvpEnabled).map((item) => item.title) : [],
        ...
        functions_attending: attending ? timeline.filter((item) => item.rsvpEnabled).map((item) => item.title) : [],
        scheduleResponses: attending ? Object.fromEntries(timeline.filter((item) => item.rsvpEnabled).map((item) => [item.id, { attending: true, titleSnapshot: item.title }])) : {},
      ...
      <AnimatePresence mode="wait">
        ...
        {view === 'dashboard' && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {experienceConfig?.sectionVisibility?.hero !== false && (
                <div 
        ...
        {view === 'invitation' && (
          ...
          >
            ...
            <BirthdayExperience family={family} currentEvent={event} experienceConfig={experienceConfig} />
          </motion.div>
        )}
                  ...
                  {attending && (
                    ...
                    >
                      ...
                      {experienceConfig?.rsvpSettings?.askAccommodation === true && (
                        <div className="flex items-center gap-3 bg-black/40 p-4 border border-amber-400/10 rounded-xl">
                          <input 
                            type="checkbox" 
                          ...
                          />
                          <label htmlFor="accommodation-stay-check" className="text-xs text-[#FDFBF7]/85 cursor-pointer select-none">
                      ...
                      {experienceConfig?.customQuestions && experienceConfig.customQuestions.length > 0 && (
                        <div className="space-y-4 pt-4 border-t border-purple-500/10">
                          <h4 className="text-[#FDFBF7]/80 text-[10px] uppercase tracking-wider font-mono font-bold">Custom Questions</h4>
                            ...
                            <div key={q.id} className="flex flex-col gap-1.5">
                              ...
                              {q.type === 'long' && (
                                <textarea
                                  ...
                                  disabled={isRsvpLocked}
                                  rows={3}
                                  className="w-full bg-black/40 border border-amber-400/15 focus:border-amber-400 rounded-xl p-3 text-cream outline-none transition-all leading-relaxed"
                                  value={customAnswers?.[q.id] || ''}
                                  onChange={e => setCustomAnswers(prev => ({
                                    ...prev,
                                    [q.id]: e.target.value
                                    ...
                                    <button
                                      ...
                                      className={`py-2 rounded-lg border text-center font-bold tracking-wider uppercase font-mono text-[10px] transition-all cursor-pointer ${
                                        customAnswers?.[q.id] === opt 
                                          ...
                                          : 'bg-black/40 border-amber-400/10 text-[#FDFBF7]/60 hover:border-amber-400/25'
                                      }`}
                                    >
                                      {opt}
                                    </button>
