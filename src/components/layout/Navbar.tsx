...
export default function Navbar() {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-[1000] transition-all duration-500 flex items-center justify-between px-6 sm:px-10 lg:px-20
          ${isScrolled 
            ? 'bg-dark-2/95 backdrop-blur-2xl border-b border-gold/20 py-3 sm:py-4' 
        ...
        >
          <Link to="/" className="flex items-center gap-2.5 sm:gap-4 group">
            <img 
              ...
              className="w-10 h-10 sm:w-11 sm:h-11 md:w-[3rem] md:h-[3rem] object-contain transition-transform duration-300 group-hover:scale-105"
              referrerPolicy="no-referrer"
            />
            <h1 className="hidden sm:block font-serif text-sm md:text-base lg:text-lg xl:text-xl tracking-[0.15em] lg:tracking-[0.2em] uppercase text-gold text-nowrap">Eventra Occasionz</h1>
          </Link>
        </motion.div>
        ...
        >
          {authType ? (
            ...
            <motion.button
              ...
              className="hidden lg:inline-block px-4 py-1.5 bg-gold/5 border border-gold/40 rounded-full text-[10px] text-gold uppercase tracking-[0.15em] hover:bg-gold hover:text-dark font-mono transition-all cursor-pointer shadow-[0_4px_15px_rgba(201,168,76,0.1)]"
            >
              Client Portal
            </motion.button>
          )}
          ...
          <motion.button
            ...
            className="lg:hidden text-gold focus:outline-none p-1.5 hover:bg-white/5 rounded-full transition-colors"
          >
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-dark-2/98 backdrop-blur-2xl z-[990] flex flex-col items-center justify-center gap-6 overflow-y-auto py-24 px-6"
          >
            <img 
              src={eventraLogo} 
              alt="Eventra Occasionz Logo" 
            ...
            {authType && (
              <div className="text-center bg-gold/5 border border-gold/20 p-4 rounded-xl w-4/5 max-w-xs mb-2 hover:border-gold/40 transition-colors">
            ...
            {!authType && (
              <button
                ...
                className="px-6 py-2.5 border border-gold text-gold text-xs uppercase tracking-widest rounded-full cursor-pointer bg-gold/5 font-mono hover:bg-gold hover:text-dark transition-all"
              >
                Client Portal Login
              </button>
            )}
            ...
            {isAdmin && (
              <Link
                to="/admin"
                onClick={closeMenu}
                className="text-gold uppercase tracking-widest text-xs mt-4 font-mono border-t border-gold/10 pt-4 w-1/2 text-center hover:text-gold-light transition-colors"
              >
                Admin Controls
              </Link>
            )}
